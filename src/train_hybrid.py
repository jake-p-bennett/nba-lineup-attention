"""
Training script for hybrid models (stats + residual embeddings).

Usage:
    python src/train_hybrid.py --model baseline
    python src/train_hybrid.py --model attention
"""
...
DATA_DIR = Path(__file__).parent.parent / "data" / "games_hybrid_processed"
...
def train(
    ...
    for epoch in range(epochs):
        ...
        if val_metrics["rmse"] < best_val_rmse:
            ...
            }, CHECKPOINT_DIR / f"hybrid_{model_type}{suffix}_best.pt")
        else:
    ...
    checkpoint = torch.load(CHECKPOINT_DIR / f"hybrid_{model_type}{suffix}_best.pt")
    ...
    print(f"\n{'='*60}")
    print(f"TEST RESULTS (hybrid_{model_type}{suffix})")
    print(f"{'='*60}")
    ...
    # Save results
    results = {
        "model_type": f"hybrid_{model_type}{suffix}",
    ...
    with open(CHECKPOINT_DIR / f"hybrid_{model_type}{suffix}_results.json", "w") as f:
